#!/usr/bin/env python3
"""
Uruchamia kolejno dla wszystkich plików .txt w katalogu 'task4' (alfabetycznie):
  <python> task4/task4.py -i <plik>
"""
import sys
import subprocess
...
def find_txt_files(task4_dir: Path):
    return sorted([p for p in task4_dir.iterdir() if p.is_file() and p.suffix.lower() == ".txt"])
...
def main():
    parser = argparse.ArgumentParser(description="Uruchamia task4.py (bfs, dfs) dla wszystkich plików .txt w ./task4 i mierzy czas.")
    ...
    if not task4_dir.exists() or not task4_dir.is_dir():
        print(f"Nie znaleziono katalogu: {task4_dir}", file=sys.stderr)
        sys.exit(1)

    txt_files = find_txt_files(task4_dir)
    if not txt_files:
        print(f"Brak plików .txt w katalogu: {task4_dir}")
        return

    task4_py = task4_dir / "task4.py"
    if not task4_py.exists():
        print(f"Nie znaleziono pliku {task4_py}", file=sys.stderr)
        sys.exit(1)
    ...
    for txt in txt_files:
        ...
        cmd = [py_exec, str(task4_py), "-i", str(txt)]
        print(f">>> Uruchamiam")
