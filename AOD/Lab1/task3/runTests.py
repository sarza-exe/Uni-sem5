#!/usr/bin/env python3
"""
Uruchamia kolejno dla wszystkich plików .txt w katalogu 'task3' (alfabetycznie):
  <python> task3/task3.py -i <plik>
"""
import sys
import subprocess
...
def find_txt_files(task3_dir: Path):
    return sorted([p for p in task3_dir.iterdir() if p.is_file() and p.suffix.lower() == ".txt"])
...
def main():
    parser = argparse.ArgumentParser(description="Uruchamia task3.py (bfs, dfs) dla wszystkich plików .txt w ./task3 i mierzy czas.")
    ...
    if not task3_dir.exists() or not task3_dir.is_dir():
        print(f"Nie znaleziono katalogu: {task3_dir}", file=sys.stderr)
        sys.exit(1)

    txt_files = find_txt_files(task3_dir)
    if not txt_files:
        print(f"Brak plików .txt w katalogu: {task3_dir}")
        return

    task3_py = task3_dir / "task3.py"
    if not task3_py.exists():
        print(f"Nie znaleziono pliku {task3_py}", file=sys.stderr)
        sys.exit(1)
    ...
    for txt in txt_files:
        ...
        cmd = [py_exec, str(task3_py), "-i", str(txt)]
