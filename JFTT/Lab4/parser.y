...
struct VariableInfo {
    std::string name;
    Symbol *sym;
    Symbol *ref; // zapisujemy symbol referencji np. x dla arr[x]
    bool is_param;
...
void semantic_error(long long lineno, char const *s) {
...
command:
    ...
    | proc_call SEMICOLON {
        ...
        long long procLable = symbolTable.getProcedureLable($1->id->pid);
