%{
    ...
    // Oblicza modulo dla wykładnika (mod P-1)
    long long modulo_phi(long long a) {
        long long phi = P - 1;
        long long res = a % phi;
        if (res < 0) res += phi;
        return res;
    ...
    // Funkcja naprawiająca bufor dla wykładnika raw_exp
    void fix_exponent_display(long long raw_exp) {
        long long val_p = modulo(raw_exp); // co jest teraz w buforze
        long long val_phi = modulo_phi(raw_exp); // co powinno być

        if (val_p == val_phi) return;

        char wrong_str[64];
        sprintf(wrong_str, "%lld ", val_p);
        int wrong_len = strlen(wrong_str);

        int buf_len = strlen(buffer);

        if (buf_len >= wrong_len && strcmp(buffer + buf_len - wrong_len, wrong_str) == 0) {
            buffer[buf_len - wrong_len] = '\0'; //Usuwamy starą liczbę 
            append_num(val_phi);
        }
    }
...
%token COMMENT;
%token EOL;
%token ERR; // token nieuzywany, lex zwraca gdy dostaje cos niepoprawnego
%left '+' '-';
%left '*' '/';
%nonassoc '^';
