...
\begin{enumerate}
    ...
    \item Dla $\operatorname{cond}(A) > 10^{12}$ wyniki mogą nie być wiarygodne bez dodatkowych zabiegów.
\end{enumerate}
...
Zadanie: obliczyć miejsca zerowe wielomianu Wilkinsona w postaci naturalnej $P(x)$ za pomocą funkcji \texttt{roots} z pakietu \texttt{Polynomials} z Julii. Porównać otrzymane pierwiastki $z_k$ z pierwiastkami „oczekiwanymi” $k=1,\dots,20$ przez obliczenie wartości $|P(z_k)|, |p(z_k)|$ (ewaluacja przez iloczyn $(x-1)\cdots(x-20)$) oraz $|z_k-k|$. Potem powtórzenie eksperymentu po maleńkiej perturbacji współczynnika $-210$ o $-2^{-23}$.
...
\begin{table}[H]
\centering
\caption{Wyniki bez perturbacji: pierwiastki są bliskie wartościom właściwym, ale można zauważyć duże błędy przy obliczaniu wartości funkcji $|P(z_k)|$ i $|p(z_k)|$. Sugeruje to, że problem wyznaczania pierwiastków tego wielomianu jest źle uwarunkowany.}
...
\begin{itemize}
    ...
    \item \texttt{Float32} z jednorazowym \textbf{obcięciem} po 10. iteracji (odrzucenie cyfr po trzecim miejscu po przecinku),
    \item \texttt{Float64}.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Model logistyczny przy ($r=3$) jest numerycznie niestabilny — niewielkie błędy kumulują się dając duże rozbieżności końcowe. Wynika to z czułości na drobne zmiany wartości i precyzję arytmetyki.
\end{enumerate}
...
\hline
\multirow{2}{*}{iter} & (case 1) & (case 2) & (case 3) & (case 4) & (case 5) & (case 6) & (case 7) \\
& $c=-2, x_0=1$ & $c=-2, x_0=2$ & $c=-2, x_0=1.99...9$ & $c=-1, x_0=1$ & $c=-1, x_0=-1$ & $c=-1, x_0=0.75$ & $c=-1, x_0=0.25$ \\
\hline
...
7 & -1.0000000000 & 2.0000000000 & 1.9999999998 & 0.0000000000 & 0.0000000000 & -0.0989561875 & -0.9999971292 \\
...
12 & -1.0000000000 & 2.0000000000 & 1.9999998324 & -1.0000000000 & -1.0000000000 & -0.9999994232 & 0.0000000000 \\
...
\hline
\end{tabular}
\caption{Iteracje ciągu $x_{n+1}=x_n^2+c$ dla różnych parametrów (precyzja Float64).}
\label{tab:iterations_full}
\end{table}

\subsection{Wnioski}

\begin{enumerate}
    \item Proces generowania ciągów równaniem rekurencyjnym ma różną stabilność w zależności od przyjętych parametrów.
    
    \item Dla $c=-2$ starty dokładnie w ($2$) lub w ($-1$) dają stałe ciągi (cases 1--2). Natomiast starty bardzo blisko punktu niestabilnego (case 3) \textbf{szybko odchodzą} od niego: maleńka różnica rośnie i prowadzi do dużych zmian --- przykład silnej wrażliwości (numeryczna niestabilność).
    
    \item Dla $c=-1$ obserwujemy zachowanie zbieżności do \textbf{przyciągającego cyklu okresu 2} $(0, -1)$. Z różnych początków (cases 4--7) ciągi wchodzą w ten cykl albo w niego od razu trafiają.
\end{enumerate}
