...
% --- PREAMBUŁA ---
% Kodowanie i czcionki
\usepackage{fontspec}
\setmainfont{Latin Modern Roman}
\usepackage[polish]{babel} % Ustawienia dla języka polskiego (np. Rysunek zamiast Figure)

% Pakiety matematyczne
\usepackage{amsmath}       % Zaawansowane środowiska matematyczne
\usepackage{extarrows}     % Dla \xlongequal

% Grafika i układ
\usepackage{graphicx}      % Do wstawiania obrazków
\usepackage[margin=1in]{geometry} % Ustawienie marginesów
\usepackage{float}         % Dla opcji [H] (wstaw "tutaj")
\usepackage{subcaption}    % Do wstawiania obrazków obok siebie (subfigure)
\usepackage{array}         % Dla zaawansowanych tabel (choć tu proste)

% Inne
\usepackage{hyperref}      % Tworzy klikalne linki w PDF (opcjonalne)
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
...
% --- Strona tytułowa ---
\title{Sprawozdanie z Listy nr 2}
...
Rozwiązanie znajduje się w pliku \texttt{task1.jl}.
...
\texttt{Float32} w tym przykładzie „zatopił” perturbację zaokrągleniami — wynik nie zmienił się widocznie. \texttt{Float64} był wystarczająco precyzyjny, by wykryć różnicę i wynik przesunął się o ~$4.3 \times 10^{-3}$.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{plots/desmos-graph.png}
\caption{Wizualizacja funkcji $f$ z użyciem kalkulatora graficznego Desmos}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{plots/geogebra-export(2).png}
\caption{Wizualizacja funkcji $f$ z użyciem kalkulatora graficznego Geogebra}
\end{figure}
...
Gdy $e^{-x}$ podpełnia do $0$, $\ln(1+e^{-x})$ przyjmuje wartość $0$ i mnożenie przez (zawodny) $e^x$ może dać $0$ lub $\text{NaN/Inf}$ — stąd skoki i „oscylogramy”.
...
Zadanie polega na rozwiązaniu układów $Ax=b$ dla dwóch rodzin macierzy: Hilberta $H_n$ dla rosnącego $n$ oraz losowych macierzy $R_n$ wygenerowanych z zadanym wskaźnikiem uwarunkowania $c$. Dla każdej macierzy rozwiązano układ dwoma metodami: \texttt{A\string\b} oraz \texttt{inv(A)*b}. Porównano otrzymane wektory z rozwiązaniem dokładnym, licząc względne błędy $\mathrm{rel}_2$, oraz sprawdzono $\operatorname{cond}(A)$ i $\operatorname{rank}(A)$.
...
\hline
$n$ & $\operatorname{cond}(A)$ & $\operatorname{rank}(A)$ & $\mathrm{rel}_2(\text{\texttt{A\string\b}})$ & $\mathrm{rel}_2(\text{\texttt{inv(A)*b}})$ \\
\hline
...
$\mathrm{rel}_2(\text{\texttt{A\string\b}})$ i $\mathrm{rel}_2(\text{\texttt{inv(A)*b}})$ — względne błędy w normie 2 dla dwóch metod rozwiązania (kolejno: \texttt{A\string\b}, \texttt{inv(A)*b}).

Interpretacja: rosnące $\operatorname{cond}(A)$ koreluje z narastaniem błędu; \texttt{A\string\b} zwykle daje mniejsze błędy niż \texttt{inv(A)*b}.
...
\hline
$n$ & $c$ & $\operatorname{cond}(A)$ & $\operatorname{rank}(A)$ & $\mathrm{rel}_2(\text{\texttt{A\string\b}})$ & $\mathrm{rel}_2(\text{\texttt{inv(A)*b}})$ \\
\hline
...
\begin{enumerate}
    \item Dla macierzy Hilberta błąd rośnie bardzo szybko wraz ze wzrostem $n$ — problem staje się silnie źle uwarunkowany (w tabeli już od $n \approx 10$ widać wyraźny wzrost błędów).
    \item \texttt{A\string\b} daje zwykle mniejsze błędy niż \texttt{inv(A)*b} — odwracanie macierzy jest mniej stabilne numerycznie.
    \item Wzrost $\operatorname{cond}(A)$ koreluje z rosnącym błędem; ekstremalnie duże wartości $\operatorname{cond}$ prowadzą do błędów rzędu jedności lub większych.
    \item Spadek rzędu numerycznego $(\operatorname{rank}(A) < n)$ wiąże się z gwałtownym pogorszeniem jakości rozwiązania (przykład: $c$ bardzo duże $\rightarrow$ rank spada $\rightarrow$ duże relacje błędów).
    \item W praktyce: zawsze sprawdzać $\operatorname{cond}(A)$ i $\operatorname{rank}(A)$; używać \texttt{A\string\b} zamiast \texttt{inv(A)*b}; dla źle uwarunkowanych macierzy rozważyć regularyzację lub arytmetykę o większej precyzji.
    \item Dla $\operatorname{cond}(A) \gg 10^{12}$ wyniki mogą nie być wiarygodne bez dodatkowych zabiegów.
\end{enumerate}
...
Zadanie: obliczyć miejsca zerowe wielomianu Wilkinsona w postaci naturalnej $P(x)$ za pomocą funkcji \texttt{roots} z pakietu \texttt{Polynomials} z Julii. Porównać otrzymane pierwiastki $z_k$ z pierwiastkami „oczekiwanymi” $k=1,\dots,20$ przez obliczenie wartości $|P(z_k)|, |p(z_k)|$ (ewaluacja przez iloczyn $(x-1)\cdots(x-20)$) oraz $|z_k-k|$. Część (b) powtórzona po maleńkiej perturbacji współczynnika o $2^{-23}$.

Rozwiązanie znajduje się w pliku \texttt{task4.jl}.

\subsection{Wyniki}

\textbf{Opis kolumn:}
\begin{itemize}
    \item $k$ — wartość oczekiwanego pierwiastka (1..20).
    \item $z_k$ / $z_k'$ — wartość znalezionego pierwiastka
    \item $|P(z_k)|$ — wartość modułu wielomianu w postaci naturalnej.
    \item $|p(z_k)|$ — wartość modułu ewaluacji przez iloczyn $(x-1)\cdots(x-20)$.
    \item $|z_k-k|$ — przesunięcie względem oczekiwanego pierwiastka.
\end{itemize}

\subsection*{Bez perturbacji}

\begin{table}[H]
\centering
\caption{Wyniki bez perturbacji: pierwiastki są bliskie wartościom całkowitym, ale odchyłki rosną dla większych $k$. Wartości $|P(z_k)|$ i $|p(z_k)|$ (ewaluacje różnymi metodami) są znaczące — różnice wynikają z kondycji numerycznej ewaluacji.}
...
\subsection*{Po perturbacji $(-210 \to -210-2^{-23})$}

\begin{table}[H]
\centering
\caption{Po bardzo małej perturbacji współczynnika część pierwiastków przesuwa się znacznie; pojawiają się pary pierwiastków zespolonych sprzężonych (dla wyższych indeksów), a przesunięcia $|z_k'-k|$ mogą być rzędu jednostek.}
\begin{tabular}{| r | l | l | l | l |}
\hline
$k$ & $z_k'$ & $|P(z_k')|$ & $|p(z_k')|$ & $|z_k'-k|$ \\
\hline
...
\subsection{Wnioski}
\begin{enumerate}
    \item Wielomian Wilkinsona jest bardzo źle uwarunkowany — małe zmiany współczynników powodują duże przesunięcia pierwiastków.
    \item Przed perturbacją pierwiastki były bliskie wartościom całkowitym, ale odchyłki rosły wraz z $k$.
    \item Po perturbacji: część pierwiastków rozszczepiła się na pary zespolone sprzężone — to typowy efekt niestabilności: realne pierwiastki zamieniają się lokalnie w pary zespolone.
    \item Różnice $|P(z_k)|$ vs $|p(z_k)|$ pokazują, że ewaluacja w postaci współczynnikowej i iloczynowej ma różną kondycję numeryczną — wartości dużych modułów świadczą o utracie dokładności przy ewaluacji.
\end{enumerate}
...
\begin{itemize}
    \item \texttt{Float32} (bez modyfikacji),
    \item \texttt{Float32} z jednorazowym \textbf{obcięciem} po 10. iteracji (ucięcie do trzech miejsc po przecinku),
    \item \texttt{Float64}.
\end{itemize}
...
Rozwiązanie znajduje się w pliku \texttt{task5.jl}.

\subsection{Wyniki}

\paragraph{Tabela wyników (wartości $p_n$ w kolejnych iteracjach)}

\begin{table}[H]
\centering
\caption{Kolumny pokazują wartości $p_n$ w kolejnych iteracjach dla trzech wariantów obliczeń. Wersja z obcięciem została ucięta raz po 10. iteracji i kontynuowano iteracje od tej uciętej wartości.}
\begin{tabular}{| r | l | l | l |}
\hline
$n$ & \texttt{Float32} & \texttt{Float32} z obcięciem & \texttt{Float64} \\
\hline
...
\begin{enumerate}
    \item W początkowych iteracjach wartości funkcji w obu artymetykach są zbieżne.
    \item Już niewielka modyfikacja (obcięcie raz po 10. iteracji) prowadzi do \textbf{znacznie odmiennych} trajektorii. Już przy 19 iteracji model z obcięciem daje duży błąd względem pozostałych dwóch modeli.
    \item Różnice między \texttt{Float32} a \texttt{Float64} kumulują się — różnice rosną z liczbą iteracji (od iteracji 22 widać znaczne różnice w wartościach funkcji) i zachodzi efekt kumulacji błędów.
    \item Model logistyczny przy ($r=3$) jest czuły na drobne zmiany wartości i precyzję arytmetyki — niewielkie perturbacje (obcięcie lub niższa precyzja) dają duże rozbieżności końcowe.
\end{enumerate}
...
Zadanie polega na obliczaniu w arytmetyce \texttt{Float64} kolejnych wartości wyrażenia rekurencyjnego $x_{n+1}=x_{n}^2+c$. Rozwiązanie znajduje się w pliku \texttt{task6.jl}.
...
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-1_x0_-1_cobweb.png}
    \caption{$x_0 = -1.0$}
\end{subfigure}
\hfill % Space between
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-1_x0_1_cobweb.png}
    \caption{$x_0 = 1.0$}
\end{subfigure}
\caption{Graficzne iteracje ciągu $x_{n+1}=x_{n}^2-1$. Dla $x_0 = -1.0 $ ciąg: ($1\mapsto0\mapsto-1\mapsto0\mapsto-1\ldots$). Szybkie wejście w cykl okresu $(0,-1)$. Dla $x_0 = 1.0 $ już w cyklu: ($1\mapsto0\mapsto-1\ldots$). Ciąg pozostaje w okresie-2 (punkt startowy na cyklu).}
\end{figure}

\begin{figure}[H]
\centering
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-1_x0_0.75_cobweb.png}
    \caption{$x_0 = 0.75$}
\end{subfigure}
\hfill % Space between
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-1_x0_0.25_cobweb.png}
    \caption{$x_0 = 0.25$}
\end{subfigure}
\caption{Graficzne iteracje ciągu $x_{n+1}=x_{n}^2-1$. Dla $x_0 = 0.75 $ początkowo nieregularne wartości, ale w krótkim czasie następuje zbliżenie do cyklu $(0,-1)$ — w tabelce widoczne częste wartości $-1$ i $0$ od pewnej iteracji (przyciąganie cyklu). Dla $x_0=0.25$ również po kilku iteracjach następuje przejście ku okresowi-2 $(0,-1)$.}
\end{figure}

\begin{figure}[H]
\centering
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-2_x0_1_cobweb.png}
    \caption{$x_0 = 1.0$}
\end{subfigure}
\hfill % Space between
\begin{subfigure}{0.48\textwidth}
    \includegraphics[width=\linewidth]{plots/c-2_x0_2_cobweb.png}
    \caption{$x_0 = 2.0$}
\end{subfigure}
\caption{Graficzne iteracje ciągu $x_{n+1}=x_{n}^2-2$. Dla $x_0 = 1.0$ Szybkie przejście: ($1\mapsto0\mapsto-1\mapsto-1\mapsto\ldots$). Ciąg „zatrzymuje się” w $-1$ (punkt stały osiągnięty dokładnie). Dla $x_0=2.0$ ciąg równa się $2$ we wszystkich iteracjach.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{plots/c-2_x0_1.99999999999999_cobweb.png}
\caption{Start bardzo blisko $2$. Przez kilka iteracji $x_n\approx2$, potem wartości stopniowo oddalają się od $2$ i pojawiają się duże oscylacje/zmiany (przejście przez wartości $>1$, $<0$, aż do dużych ujemnych) — wrażliwość na maleńkie perturbacje.}
\end{figure}


% Tabela jest bardzo szeroka, zmniejszamy czcionkę i odstępy
\begin{table}[H]
\centering
\tiny % Najmniejsza czcionka
\setlength\tabcolsep{3pt} % Zmniejszenie odstępów między kolumnami
\begin{tabular}{| r | r | r | r | r | r | r | r |}
\hline
iter & $c=-2, x_0=1$ & $c=-2, x_0=2$ & $c=-2, x_0=1.99...9$ & $c=-1, x_0=1$ & $c=-1, x_0=-1$ & $c=-1, x_0=0.75$ & $c=-1, x_0=0.25$ \\
\hline
$1$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.43750$ & $-0.93750$ \\
$2$ & $-1.00000$ & $2.00000$ & $2.00000$ & $-1.00000$ & $-1.00000$ & $-0.80859$ & $-0.12109$ \\
$3$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.34618$ & $-0.98534$ \\
$4$ & $-1.00000$ & $2.00000$ & $2.00000$ & $-1.00000$ & $-1.00000$ & $-0.88016$ & $-0.02911$ \\
$5$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.22531$ & $-0.99915$ \\
$6$ & $-1.00000$ & $2.00000$ & $2.00000$ & $-1.00000$ & $-1.00000$ & $-0.94923$ & $-0.00169$ \\
$7$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.09896$ & $-1.00000$ \\
$8$ & $-1.00000$ & $2.00000$ & $1.99999$ & $-1.00000$ & $-1.00000$ & $-0.99021$ & $-0.00001$ \\
$9$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.01949$ & $-1.00000$ \\
$10$ & $-1.00000$ & $2.00000$ & $1.99999$ & $-1.00000$ & $-1.00000$ & $-0.99962$ & $0.00000$ \\
$11$ & $-1.00000$ & $2.00000$ & $2.00000$ & $0.00000$ & $0.00000$ & $-0.00076$ & $-1.00000$ \\
$12$ & $-1.00000$ & $2.00000$ & $2.00000$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$13$ & $-1.00000$ & $2.00000$ & $1.99999$ & $0.00000$ & $0.00000$ & $-0.00000$ & $-1.00000$ \\
$14$ & $-1.00000$ & $2.00000$ & $2.00000$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$15$ & $-1.00000$ & $2.00000$ & $1.99999$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$16$ & $-1.00000$ & $2.00000$ & $1.99996$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$17$ & $-1.00000$ & $2.00000$ & $1.99983$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$18$ & $-1.00000$ & $2.00000$ & $1.99931$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$19$ & $-1.00000$ & $2.00000$ & $1.99725$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$20$ & $-1.00000$ & $2.00000$ & $1.98902$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$21$ & $-1.00000$ & $2.00000$ & $1.95622$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$22$ & $-1.00000$ & $2.00000$ & $1.82678$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$23$ & $-1.00000$ & $2.00000$ & $1.33712$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$24$ & $-1.00000$ & $2.00000$ & $-0.21211$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$25$ & $-1.00000$ & $2.00000$ & $-1.95501$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$26$ & $-1.00000$ & $2.00000$ & $1.82206$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$27$ & $-1.00000$ & $2.00000$ & $1.31991$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$28$ & $-1.00000$ & $2.00000$ & $-0.25784$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
$29$ & $-1.00000$ & $2.00000$ & $-1.93352$ & $0.00000$ & $0.00000$ & $0.00000$ & $-1.00000$ \\
$30$ & $-1.00000$ & $2.00000$ & $1.73850$ & $-1.00000$ & $-1.00000$ & $-1.00000$ & $0.00000$ \\
\hline
\end{tabular}
\end{table}
